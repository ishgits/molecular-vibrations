"""Element data: Z -> symbol, symbol -> CPK-ish color, symbol -> covalent radius.

Covalent radii (Angstrom) from Cordero et al. (2008). Colors follow common
molecular-visualization (CPK/Jmol-like) conventions, with the original tool's
choices preserved for the elements it hard-coded (Ba/Ti/Cu perovskite palette).
"""

from __future__ import annotations

# Atomic number -> element symbol (1..118).
...
# Covalent radii in Angstrom (Cordero 2008), single-bond values.
...
# CPK/Jmol-like colors, overlaid with the original tool's explicit choices.
ELEMENT_COLOR: dict[str, str] = {
    "H": "white", "C": "black", "N": "blue", "O": "red", "F": "green",
    "P": "orange", "S": "yellow", "Cl": "green", "Br": "darkred",
    "I": "purple", "B": "salmon", "Na": "purple", "Mg": "darkgreen",
    "Ca": "darkgray", "Fe": "darkorange", "Zn": "slategray",
    # original tool's perovskite palette:
    "Ba": "green", "Ti": "gray", "Cu": "orange",
}
...
def get_element_color(symbol: str) -> str:
    return ELEMENT_COLOR.get(symbol, DEFAULT_COLOR)


def get_covalent_radius(symbol: str) -> float:
    return COVALENT_RADIUS.get(symbol, DEFAULT_RADIUS)
